...
function docsUrl(id) {
    ...
    return `https://web-platform-dx.github.io/web-features/features/${encodeURIComponent(id)}.html`;
}
function tipFor(id) {
    if (id.includes("has"))
        return "Use progressive enhancement: toggle a class in JS as a fallback.";
    if (id.includes("container"))
        return "Offer layout utility fallbacks for older engines.";
    ...
    if (id.includes("view-transition"))
        return "Guard with feature-detect; provide instant hide/show fallback.";
    if (id.includes("dialog"))
        return "Polyfill or use ARIA-compliant modal pattern.";
    if (id.includes("popover"))
        return "Consider an accessible custom popover as a fallback.";
    if (id.includes("focus-visible"))
        return "Provide focus styles gated by :focus-visible polyfill fallback.";
    if (id.includes("layer"))
        return "Gracefully degrade layering by ordering and specificity.";
